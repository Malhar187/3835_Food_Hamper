...
def xai_report():
    ...
    explainer = shap.KernelExplainer(predict_fn, data.drop(columns=["actual_pickup_boxcox"]).iloc[:1])  # Only take the first row
    shap_values = explainer.shap_values(data.drop(columns=["actual_pickup_boxcox"]).iloc[:1])  # Ensure consistent shape
