...
def machine_learning_modeling():
    ...
    if st.button("🚀 Predict"):
        ...
        # Display predicted pickups for the next week
        st.success("Predicted Pickups for the next week:")
        fig = px.line(prediction_df, x='Date', y='Predicted Pickups', 
                      title="📊 Predicted Pickups for the Next Week", markers=True)
        st.plotly_chart(fig, use_container_width=True)
...
def predict_hamper_pickups(input_features):
    ...
    start = len(data)  # Assuming you're predicting the next 7 days based on your data length
    end = start + 6  # 6 days ahead (for 7 total days)

    predictions = model.predict(start=start, end=end, exog=input_features)
    
    # Create a DataFrame to plot the predictions
    future_dates = pd.date_range(start=daily_data['pickup_date'].max(), periods=7, freq='D')
    prediction_df = pd.DataFrame({
        'Date': future_dates,
        'Predicted Pickups': predictions
    })
