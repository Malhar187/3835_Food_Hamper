...
def chatbot_page():
    ...
    if st.button("Ask"):
        if user_query:
            answer = chatbot_response(user_query, vector_db)
            st.subheader("Answer:")
            st.write(answer)
        else:
...
def load_knowledge_base():
    # Load the daily_data.csv
    daily_data = pd.read_csv("daily_data.csv")
    
    # Extract key insights or summarize the data
    avg_pickups = daily_data['actual_pickups'].mean()
    total_pickups = daily_data['actual_pickups'].sum()
    pickup_times = "Food hamper pickup times vary depending on location."
    
    # Create a knowledge base with relevant text
    docs = [
        f"Average daily hamper pickups: {avg_pickups:.2f}",
        f"Total hamper pickups: {total_pickups}",
        pickup_times,
        "Special occasions like Eid and Ramadan affect demand.",
        "The SARIMAX model predicts future hamper pickups."
    ]
...
def retrieve_info(query, vector_db):
    ...
    return retrieved_docs[0].page_content if retrieved_docs else "No relevant information found."

# Generate a chatbot response
def chatbot_response(user_query, vector_db):
    context = retrieve_info(user_query, vector_db)
    
    chat_model = ChatOpenAI(openai_api_key=st.secrets["OPENAI_API_KEY"])
    messages = [
        SystemMessage(content="You are a helpful chatbot that provides information about food hamper pickups."),
