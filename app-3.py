...
data = {
    ...
    "Family_Size": [4, 3, 5],
    "Special_Occasion_Dhu_al-Qadah": [0, 1, 0],
    "Special_Occasion_Eid_al-Adha": [1, 0, 0],
    "Special_Occasion_Eid_al-Fitr": [0, 1, 0],
    "Special_Occasion_Muharram": [0, 0, 1],
    "Special_Occasion_Rabi_al-Awwal": [0, 0, 0],
    "Special_Occasion_Rajab": [0, 0, 0],
    "Special_Occasion_Ramadan": [1, 0, 0],
    "Special_Occasion_Shaban": [0, 1, 0],
    "Season_Summer": [1, 0, 0],
    "Season_Winter": [0, 1, 0],
    "Season_Spring": [0, 0, 1]
}
...
# Example function to predict hamper pickups (simulated)
def predict_hamper_pickups(input_features):
    model = LinearRegression()
    model.fit(input_features, [10, 20, 30])  # Dummy target values
...
def rag_chatbot_page():
    ...
    if st.button("Ask", key="ask_button"):
        if user_input:
            ...
            similarity_scores = np.dot(query_embedding, corpus_embeddings.T)[0]
            best_match_idx = np.argmax(similarity_scores)
            
            # Use .iloc to get the best match row by position (integer index)
            best_match = documents.iloc[best_match_idx]  # Accessing by position
            
            # If you need to access by label (column name), use .loc
            # best_match = documents.loc[best_match_idx]  # Accessing by label (column name)

            # Example of extracting features for prediction
            input_features = pd.DataFrame({
                "scheduled_pickups": [best_match["Scheduled_Pickups"]],
                "family_size": [best_match["Family_Size"]],
                "special_occasion_Dhu_al-Qadah": [best_match["Special_Occasion_Dhu_al-Qadah"]],
                "special_occasion_Eid_al-Adha": [best_match["Special_Occasion_Eid_al-Adha"]],
                "special_occasion_Eid_al-Fitr": [best_match["Special_Occasion_Eid_al-Fitr"]],
                "season_Summer": [best_match["Season_Summer"]],
                "season_Winter": [best_match["Season_Winter"]],
                "season_Spring": [best_match["Season_Spring"]],
            })

            # Predict hamper pickups based on features
            prediction = predict_hamper_pickups(input_features)
            response = generator(f"Based on the data, the predicted number of hamper pickups is: {prediction[0]:.2f}")
