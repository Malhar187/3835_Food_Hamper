...
# Custom CSS to make the app look more polished
st.markdown("""
    <style>
        .main {
...
def exploratory_data_analysis():
    ...
    ax.set_title("Distribution of Hamper Pickups", fontsize=14)
    st.pyplot(fig)
    ...
    ax.set_title("📊 Daily Hamper Pickups Trend", fontsize=14)
    st.pyplot(fig)
...
def xai_report():
    ...
    ax.set_title("Feature Importance Based on SARIMAX Coefficients", fontsize=14)
...
def rag_chatbot_page():
    ...
    transaction_narrative = "The clients of the food hamper program have shown a strong correlation between their " \
                            "family size, location, and the special occasions like Ramadan, Eid al-Adha, and " \
                            "other festive occasions. With detailed records, we predict a higher demand during these " \
                            "events, especially when more family members are involved."
    
    # Embedding the transaction narrative to vector space
    transaction_vector = embedder.encode([transaction_narrative])
    
    # Placeholder response for query
