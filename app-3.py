...
def machine_learning_modeling():
    ...
    if st.button("🚀 Predict"):
        ...
        st.success("Predicted Pickups for the Next 7 Days")
        st.dataframe(prediction_df)

        fig = px.line(prediction_df, x='Date', y='Predicted Pickups',
                  title="📈 7-Day Hamper Pickup Forecast",
                  markers=True)
        st.plotly_chart(fig, use_container_width=True)
...
def predict_hamper_pickups(input_features):
    ...
    # Repeat input features for the next 7 days
    input_7days = pd.concat([input_features]*7, ignore_index=True)
    start = len(data)
    end = start + 6  # 7-day forecast

    forecast = model.predict(start=start, end=end, exog=input_7days)

    # Build a date range for the forecast
    last_date = pd.to_datetime(daily_data['pickup_date']).max()
    forecast_dates = pd.date_range(start=last_date + pd.Timedelta(days=1), periods=7)

    return pd.DataFrame({
        "Date": forecast_dates,
        "Predicted Pickups": forecast
    })
