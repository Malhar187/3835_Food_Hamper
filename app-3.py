...
@st.cache_resource
def load_embedder():
    return SentenceTransformer("all-MiniLM-L6-v2", device="cpu")  # or 'cuda' if available

# Cache the Flan-T5 generator (fixing meta tensor crash!)
@st.cache_resource
def load_generator():
    return pipeline(
        "text2text-generation",
        model="google/flan-t5-large",
        device=-1  # Use CPU; if GPU available, try device_map="auto"
    )
...
def rag_chatbot_page():
    ...
    for idx, row in transaction_data.iterrows():
        transaction_narrative += (
            ...
            f"{', '.join([occ for occ in ['Dhu al-Qadah', 'Eid al-Adha', 'Eid al-Fitr', 'Muharram',
                                    'Rabi al-Awwal', 'Rajab', 'Ramadan', 'Shaban']
                     if row[f'Special_Occasion_{occ.replace(' ', '_')}'] == 1])}.\n"
            f"Client's location: {row['Location']}.\n"
        )

    # Example charity info (updated according to your project focus)
    charity_info = (
