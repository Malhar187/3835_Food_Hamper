...
def xai_report():
    ...
    prediction = predict_hamper_pickups(input_features)
    st.subheader("Prediction")
    st.write(f"Predicted Pickup: {prediction[0]}")
    ...
    # Only include coefficients for exogenous variables (ignore intercept and other terms)
    exog_coeffs = coefficients_df.loc[exog_cols]
